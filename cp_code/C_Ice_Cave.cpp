...
void dfs(pi node,pi end,vector<vector<char>>&v, vector<vector<int>>& vis){
    int x=node.first;
    int y=node.second;
    int n=v.size();
    int m=v[0].size();
    ...
    for(int i=0;i<4;i++){
        ...
        if((n_row>=0 && n_row<n && n_col>=0 && n_col<m) ){
                if((vis[n_row][n_col]==0 && v[n_row][n_col]=='.')||(vis[n_row][n_col]<2 && (make_pair(n_row,n_col)==end))){
                    dfs({n_row,n_col},end,v,vis);
                }
        }
    }
}
...
pi start={start_x,start_y};
pi end={end_x,end_y};


vector<vector<int>> vis(n,vector<int> (m));
// if(start==end){cout<<"YES";return ;}


if(v[end.first][end.second]=='X' && start!=end)vis[end.first][end.second]++;

dfs(start,end,v,vis);

if(vis[end.first][end.second]>1)cout<<"YES";
else cout<<"NO";
