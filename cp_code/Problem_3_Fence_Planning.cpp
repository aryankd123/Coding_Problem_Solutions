...
void dfs(int node,int & min_row,int & min_col,int & max_row,int & max_col,vector<vector<int>>& adj,vector<int>& vis,vector<pi>&v){
    vis[node]=1;
    min_row=min(min_row,v[node].first);
    min_col=min(min_col,v[node].second);
    max_row=max(max_row,v[node].first);
    max_col=max(max_col,v[node].second);
    for(auto neigh:adj[node]){
        if(vis[neigh]==0){
            dfs(neigh,min_row,min_col,max_row,max_col,adj,vis,v);
        }
    }
}
...
if(vis[i]==0){
    ...
    dfs(i,min_row,min_col,max_row,max_col,adj,vis,v);
    int l=max_row-min_row;
    int b=max_col-min_col;
  ans=min(ans,2*(l+b));
    // else ans=min(ans,2*(l+b));
}
}
cout<<ans;
//vector<int> v;
