...
// }
// };
void dfs(int node,vector<int>& vis,vector<vector<int>>& adj,vector<int>& ans,vector<int>& parent){
    vis[node]=1;

    if(ans.size()!=0)return ;
    for(auto neighbours:adj[node]){
        if(vis[neighbours]==1 && neighbours==parent[node])continue;
        if(vis[neighbours]==1 && neighbours!=parent[node]){
            int flag=neighbours;
            int flag_node=node;
            while( flag_node!=flag && flag_node!=-1){
                ans.push_back(flag_node);
                flag_node=parent[flag_node];
            }
            ans.push_back(flag);
           return ;
        }
            ...
            dfs(neighbours,vis,adj,ans,parent);
            if(ans.size()!=0)return ;
        }
    }
}
