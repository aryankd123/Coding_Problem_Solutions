...
int flag=0;
vector<int> flag2=ans;
sort(flag2.begin(),flag2.end());

for(auto it:mp){
    int cnt = flag2.end() - lower_bound(flag2.begin(), flag2.end(), it.first);

    if(cnt>=2 && (ans[n-it.first]==it.first))flag=max(it.first,flag);
}
