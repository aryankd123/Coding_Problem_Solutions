...
void make_factors(int a,set<int>& st){
  for(int i=1;i*i<=a;i++){
    if(a%i==0){st.insert(i),st.insert(a/i);}
  }
}
void solve(){
int n;
...
for(int i=0;i<n;i++){
if(s[i]!=a)st.insert(i+1);

}
if(st.size()==0){cout<<0<<'\n';return ;}
for(auto it:st){
  make_factors(it,factors);
}
vector<int> factorr(n+1,0);
for(auto it:factors){
  factorr[it]=1;
}
for(int i=1;i<=n;i++){
  if(factorr[i]==0){cout<<"1\n";cout<<i<<'\n';return ;}
}
cout<<2<<'\n';
cout<<n<<" "<<n-1<<'\n';
