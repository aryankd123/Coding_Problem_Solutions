...
int flag2=0;
for(int i=0;i<n;i++){
    if(v[i]!=v2[i]){flag1=v[i];flag2=v2[i];}
}
vector<int> v3;
vector<int> v3_cpy;
vector<int> v4;
...
int flag=0;
for(int i=0;i<n;i++){
    if(v[i]!=flag1){v3.push_back(v[i]);v3_cpy.push_back(v[i]);}

}
for(int i=0;i<n;i++){
    if(v2[i]!=flag2){v4.push_back(v2[i]);v4_cpy.push_back(v2[i]);}

}
reverse(v3_cpy.begin(),v3_cpy.end());
reverse(v4_cpy.begin(),v4_cpy.end());
for(int i=0;i<v3.size();i++){
    if(v3[i]!=v3_cpy[i]){flag+=1;break;}
}
for(int i=0;i<v4.size();i++){
    if(v4[i]!=v4_cpy[i]){flag+=1;break;}
}
if(flag>=2)cout<<"NO\n";
else cout<<"YES\n";
