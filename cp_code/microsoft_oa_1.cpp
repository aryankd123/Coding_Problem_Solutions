...
cin>>x;
v.push_back(x);
if(x<0)v_neg.push_back(x);
if(x>=0)v_pos.push_back(x);
}
...
vector<int>v_pos2;
for(int i=0;i<v_pos.size();i++){
    if(v_pos[i]!=0)v_pos2.push_back(v_pos[i]);
...
vector<int>final;
int sum=0;
while(pnt_pos<v_pos.size()){
    final.push_back(v_pos[pnt_pos]);
    sum+=v_pos[pnt_pos];
    pnt_pos++;
    while(pnt_neg<v_neg.size()){
        if(sum+v_neg[pnt_neg]>0){sum+=v_neg[pnt_neg];
        final.push_back(v_neg[pnt_neg]);
        pnt_neg++;}
        else break;
    }
}
