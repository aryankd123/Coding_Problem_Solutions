...
void bfs(int start,vector<vector<int>>& adj,int n,int & ans){
    ...
    while(!q.empty()){
        ...
        for(auto neigh:adj[node]){
            if(vis[neigh]==1 && parentt!=neigh){
                ans=min(ans,dis[neigh]+dis[node]+1);
            }
            else if(vis[neigh]==0 ){
                q.push({{neigh,distance+1},node});
                parent[neigh]=node;
                dis[neigh]=distance+1;
                vis[neigh]=1;
