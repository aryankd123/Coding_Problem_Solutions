...
bool valid(int x,int y,int x1,int y1, int x2,int y2){
...
// minb=min(minb,abs(x-x2)+abs(y-y2));
// }
// cout<<min(minb,abs(x1-x2)+(y1-y2))<<'\n';
// return ;
// }
// if(b<=k){
//     for(int i=0;i<k;i++){
//    int x=v[i].first;
//     int y=v[i].second;
//       mina=min(mina,abs(x-x1)+abs(y-y1));
//     }
// cout<<min(mina,abs(x1-x2)+(y1-y2))<<'\n';
// return ;
// }
...
// if(flag==0)cout<<abs(x1-x2)+abs(y1-y2)<<'\n';
cout<< min(abs(x1-x2)+abs(y1-y2),mina+minb)<<'\n';
