...
bool dfs(int node, int team ,vector<int>& vis,vector<vector<int>>& adj){
    for(auto neighbours:adj[node]){
        if(vis[neighbours]==-1){
            vis[neighbours]=!team;
            if(!dfs(neighbours,!team,vis,adj))return 0;
            
        }
        else {
            if(vis[neighbours]==team)return 0;
...
for(int i=1;i<=n;i++){
    if(vis[i]==-1){
        vis[i]=0;
        if(!dfs(i,0,vis,adj)){cout<<"IMPOSSIBLE";return ;}
    }
}
for(int i=1;i<=n;i++){
    cout<<vis[i]+1<<" ";
