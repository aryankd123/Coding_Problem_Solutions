...
for(int i=1;i<n+1;i++){
    for(int j=1;j<n+1;j++){
        if(array[i-1][j-1]=='*'){forest[i][j]=forest[i-1][j]+forest[i][j-1]-forest[i-1][j-1]+1;}
        else {forest[i][j]=forest[i-1][j]+forest[i][j-1]-forest[i-1][j-1];}
...
//if(forest[x1-1][y1-1]=='*')lhs--;

int ans=forest[x2][y2]-forest[x2][y1-1]-forest[x1-1][y2];
ans+=forest[x1-1][y1-1];
// cout<<forest[x2][y2]<<'\n';
// cout<<lhs;
cout<<abs(ans)<<'\n';
}
