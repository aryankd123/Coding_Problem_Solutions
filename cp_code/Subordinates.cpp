...
// // }
// // };
// int dfs(int node,vector<vector<int>>&adj,vector<int>&sub,vector<int>& vis){
//     vis[node]=1;
//     int sum=0;
//     for(auto neighbours:adj[node]){
//         if(vis[neighbours]==0){
//           sum+=(1+dfs(neighbours,adj,sub,vis));
//         }
...
int dfs(int node,vector<vector<int>>& adj,vector<int> &vis,int sub,vector<int>& servants){
    vis[node]=1;
    int sum=0;
    for(auto neigh:adj[node]){
        if(vis[neigh]==0){sum+=1+(dfs(neigh,adj,vis,sub,servants));}
    }
    servants[node]=sum;
    return sum;
}
...
vector<int> vis(n+1,0);
vector<int>servants(n+1,0);
for(int i=1;i<=n;i++){
    int sub=0;
if(vis[i]==0)dfs(i,adj,vis,sub,servants);
}
for(int i=1;i<=n;i++){
cout<<servants[i]<<" ";
}
