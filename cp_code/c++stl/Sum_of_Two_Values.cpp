...
if((pos.find(sum-(it->first)) !=pos.end())  ){
    if(it->first == pos.find(sum-(it->first))->first){if(it->second.size()==1)continue;
                                                    else {
                                                        cout<<(it->second)[0]<<" "<<(it->second)[1];return;
                                                    }
                                                        
                                                        
                                                        }
    else {cout<<it->second[0]<<" "<<pos[sum-(it->first)][0];return;}   
}
}
cout<<"IMPOSSIBLE";
//vector<int> v;
