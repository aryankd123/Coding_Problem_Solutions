...
while(!pq.empty()){
    ...
    for(auto neighbours:adj[node]){
        if(dis[neighbours.first]>distance+neighbours.second){
            dis[neighbours.first]=distance+neighbours.second;
            parent[neighbours.first]=node;
            pq.push({distance+neighbours.second,neighbours.first});
...
while(parent[flag]!=flag){
auto it =adj[parent[flag]].lower_bound({flag,INT_MIN});
ans.push_back(it->second);

// ans.push_back(parent[flag].second);
flag=parent[flag];
