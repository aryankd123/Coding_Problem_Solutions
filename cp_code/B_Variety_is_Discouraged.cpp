...
void solve(){
 ...
 set<int>un_fix;
    for(auto&[x,y]:f){if(y==1){un_fix.insert(x);}}
 int right=0,lll=-1,upp=-1;
for(int left=0;right<n;right++){
if(un_fix.count(v[right])==0){left=right+1;}
else{int r=right-left+1;if(r>lll){lll=r;upp=right;} }}

    int freq_fixed=n-f.size();
 if(lll==0 || lll==-1){cout<<"0\n";}
else{cout<<upp-lll+2<<" "<<upp+1<<"\n";}
}
