...
queue<pair<pi,int>> q;
for(int i=0;i<k;i++){
q.push({{stations[i],0},parents[stations[i]]});
...
while(!q.empty()){
    ...
    for(auto neigh: adj[x]){
        if(visited[neigh.first]==1 && parents[neigh.first]!=x&& parents[x]!=neigh.first ){ans.insert(neigh.second);}
        else {
           if(dis[neigh.first]>distance+1){ q.push({{neigh.first,distance+1},x});
            dis[neigh.first]=distance+1;
            parents[neigh.first]=x;
        }
