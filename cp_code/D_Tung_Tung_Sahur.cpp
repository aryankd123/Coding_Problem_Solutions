...
if(pt!=p.length()-1){
    ...
    while(p[pt]==p[pt+1]){pt++;ctp++;}
    while(q[qt]==q[qt+1]){qt++;ctq++;}
    if(ctp>ctq){cout<<"NO\n";return;}
    if(ctq>2*ctp){cout<<"NO\n";return;}
}
if(pt==p.length()-1){
     while(q[qt]==q[qt+1]){qt++;ctq++;}
    if(ctp>ctq){cout<<"NO\n";return;}
...
if(pt==p.length()-1 && qt!=q.length()-1){
    while(qt!=q.length()-1){if(p[pt]!=q[qt]){cout<<"NO\n";return;}qt++;}
}

if(pt==p.length()-1){if(p[pt]!=q[qt]){cout<<"NO\n";return;}}
if(pt==p.length()-1){if(p[pt]!=q[q.length()-1]){cout<<"NO\n";return;}}
if(p[p.length()-1]!=q[q.length()-1]){cout<<"NO\n";return;}
pt+=1;
qt+=1;
}
cout<<"YES\n";
