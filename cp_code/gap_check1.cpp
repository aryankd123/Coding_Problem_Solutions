...
if(v[0][0]=='1' || v[n][n]=='1'){dp[1][1]=INT_MAX;dp[n][n]=INT_MAX;cout<<-1;return;}

else dp[1][1]=0;
for(int i=1;i<=n;i++){
    for(int j=1;j<=n;j++){
        if(i==1 && j==1){continue;}
        if(v[i-1][j-1]=='1'){dp[i][j]=INT_MAX;}
        
        else {
        if(i>1)dp[i][j]=min(dp[i][j],dp[i-1][j],dp[i-1][j-1],dp[i-1][j+1],dp[i+1][j+1]);
        if(j>1)dp[i][j]=min(dp[i][j],dp[i][j-1],dp[i-1][j-1],dp[i+1][j+1],dp[i-1][j+1]);
