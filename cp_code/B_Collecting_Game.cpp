...
int ind=0;
for(int i=0;i<n;i++){
    if(flag>=vp[i].first){index.push_back(ind);flag=prefix[i];}
    else {ind=i;index.push_back(ind);flag=prefix[i];}
}

vector<int> final(n,0);

for(int i=0;i<n;i++){
    auto  kk=upper_bound(index.begin(),index.end(),i)-index.begin();
    final[vp[i].second]=kk;
}
