...
if(dist[0]==0){ans+=0;}
else if(dist[0]==1){ans+=0;day+=1;}
else if(dist[0]==2){ans+=1;day+=1;}
else {ans+=1;day+=2;}
// cout<<ans<<" ";
for(int i=1;i<dist.size();i++){
    if((2*day)+1==dist[i]){ans+=(2*day);day+=1;}
    else if((2*day)+2==dist[i]){ans+=((2*day)+1);day+=1;}
    else if((2*day)+2<dist[i]){ans+=((2*day)+1);day+=2;}
    else ans+=dist[i];
