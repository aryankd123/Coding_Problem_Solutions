...
vector<string> gray(int n){
    ...
    vector<string> prev_gray=gray(n-1);
    vector<string>rev_gray=prev_gray;
    reverse(rev_gray.begin(),rev_gray.end());

    int ss=prev_gray.size();
    vector<string>res;
    for(int i=0;i<ss;i++){
        prev_gray[i]="0"+prev_gray[i];
        res.push_back(prev_gray[i]);
        rev_gray[i]="1"+rev_gray[i];

    }
res.insert(res.end(),rev_gray.begin(),rev_gray.end());
return res;
