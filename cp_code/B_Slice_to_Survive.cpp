...
int n,m,a,b;
cin>>n>>m>>a>>b;
int r1=1+min(a-1,n-a),
r2=n,c1=m,
c2=1+min(b-1,m-b);
int x=r1,r_s1=0;
    while(x>1){if(x==3){x=1;r_s1+=2;}
 else{x=(x%2==0?x/2:x/2+1);r_s1++;}
    }
    x=c1;
    int c_s1=0;
    while(x>1){
             if(x==3){x=1;c_s1+=2;}
 else{x=(x%2==0?x/2:x/2+1);c_s1++;}
    }
int ans=1+r_s1+c_s1;
 x=r2;
int r_s2=0;
while(x>1){
     if(x==3){x=1;r_s2+=2;}
     else{x=(x%2==0?x/2:x/2+1);r_s2++;}
    }
 x=c2;
int c_s2=0;
while(x>1){
     if(x==3){x=1;c_s2+=2;}
 else{x=(x%2==0?x/2:x/2+1);c_s2++;}
    }
    ans=min(ans,1+r_s2+c_s2);
    cout<<ans<<'\n';
}
