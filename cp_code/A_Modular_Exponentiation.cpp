...
//****recursive_code***
int bin_exp_rec(int a,int b,int M){a=a%M;if(b==0)return 1;long long   res=bin_exp_rec(a,b/2,M);    //to reduce the no of function calls we are precalculating the res otherwise putting it in return would increase the time complexity very much 
if (b&1)return (a* (res * res)%M)%M;else return (res*res)%M;}
//****iterative_code***
// int bin_exp_iter(int a,int b,int M){a=a%M;int ans=1;while(b){if(b&1) ans=(bin_multiply(ans,a));
