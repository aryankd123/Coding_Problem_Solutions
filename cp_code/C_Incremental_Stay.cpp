...
prefix_even.push_back(0);
int sum=0;
for(int i=1;i<=2*n;i++){
    if(i%2==1)sum+=v[i];
    else sum-=v[i];
    prefix_even.push_back(sum);

}
vector<int>prefix_odd;
prefix_odd.push_back(0);
int sum2=0;
for(int i=1;i<=2*n;i++){
    if(i%2==0)sum2+=v[i];
    else sum2-=v[i];
    prefix_odd.push_back(sum2);
...
for(int k=1;k<=n;k++){
    int ans=0;
    if(k%2 ==1){
        ans+=prefix[2*n]-prefix[2*n-k]-(prefix[k]-prefix[0]);
        ans+=prefix_odd[2*n-k]-prefix_odd[k];
    }
    else {
        ans+=prefix[2*n]-prefix[2*n-k]-(prefix[k]-prefix[0]);
        ans+=prefix_even[2*n-k]-prefix_even[k];
    }
    cout<<ans<<" ";
}
cout<<"\n";
