...
bool is_valid__(vector<int>&vv,int mid,int nn){
    // for(auto val:vv)cout<<val<<" ";
    // cout<<'\n';
    vv.push_back(nn+1);
    int n=vv.size();
    int cnt=1;
    ...
    for(int i=0;i<n;i++){
        ...
        else if(vv[i]-start-1>mid){
            if(vv[i]-start-1>2*mid+1)return 0;
            else if(cnt>0){cnt--;start=vv[i];}
...
bool isvalid(vector<vector<int>>&v1,int& mid,int& k,int& n){
    
    for(int i=1;i<=k;i++){
        if(is_valid__(v1[i],mid,n))return 1;
    }
    return 0;
}
...
vector<vector<int>>v1(k+1);
for(int i=0;i<n;i++){
v1[v[i]].push_back(i+1);
}
int low=0;
int high =2e5+10;
int ans=INT_MAX;
