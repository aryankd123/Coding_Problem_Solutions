...
void dfs(int node,map<int,set<int>>&mp,set<int>&visited){
    visited.insert(node);
...
int flag=0;
for(auto it:mp){
    if((int)it.second.size()>maxi)flag=it.first;
maxi=max(maxi,(int)it.second.size());
}

for(auto it2:mp[flag]){
    mp[it2].erase(flag);
}
mp.erase(flag);
...
int flag2=0;
for(auto it:mp){
    if((int)it.second.size()>maxi2)flag2=it.first;
maxi2=max(maxi2,(int)it.second.size());
}

for(auto it2:mp[flag2]){
    mp[it2].erase(flag2);
}
mp.erase(flag2);
