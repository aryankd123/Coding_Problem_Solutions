...
if(i!=n-1 && (v[i+1]<v[i] && ct==0)){
    ct++;
    flagl=i;
}

else if(i!=n-1 && v[i+1]>v[i] && ct==1){
flagr=i;  
ct=2;  
}
...
if(flagl!=-1 ){
    if(flagr==-1){
        reverse(v.begin()+flagl,v.end());

    }
    else {
        reverse(v.begin()+flagl,v.begin()+flagr+1);
    }
}
// cout<<flagl<<" "<<flagr<<'\n';
// for(auto val: v){
...
if (is_sorted(v.begin(),v.end())){
    ...
    cout<<"yes\n";
  if(flagr!=n)  cout<<flagl+1<<" "<<flagr+1;
  else cout<<flagl+1<<" "<<flagr;
}
