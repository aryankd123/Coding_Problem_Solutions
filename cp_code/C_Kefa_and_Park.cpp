...
void dfs(int node,vector<vector<int>> &adj,vector<int>&vis,vector<int>&cats,int& k,int contin,int & ans){
    vis[node]=1;
    int flag=0;
    for(auto neigh:adj[node]){
        ...
        if(vis[neigh]==0 ){
            flag++;
            if(contin+cats[neigh]<=k){
                if(cats[neigh]==0)dfs(neigh,adj,vis,cats,k,0,ans);
                else dfs(neigh,adj,vis,cats,k,contin+cats[neigh],ans);}
            // else if(contin+cats[neigh]>k)return ;
        }
    }
    if(flag==0)ans++;
}
...
dfs(1,adj,vis,cats,k,cats[1],ans);
