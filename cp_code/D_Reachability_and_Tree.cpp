...
// int depthOfTree(int node,vector<vector<int>>& adj,vector<int>& vis) {
//     vis[node] =1;
//     int depth =0;
//     for (int child :adj[node]) {
//         if (!vis[child]) {
//             depth =max(depth, 1 + depthOfTree(child, adj, vis));
//     }
//     }
//     return depth;
// }

void dfs(int index,bool dir,vector<vector<int>>& adj,vector<pair<int,int>> &ans,vector<int>& vis){
    vis[index]=1;
...
for(int i=1;i<=N;i++){
if(adj[i].size()==2){flag++;index=i;break;}
}
...
vis[index]=1;
ans.push_back({adj[index][1],index});
ans.push_back({index,adj[index][0]});
dfs(adj[index][0],0,adj,ans,vis);
dfs(adj[index][1],1,adj,ans,vis);
